...
int my_comparer(const void *this, const void *that)
{
  ...
  //cast this and that into Process type
  Process p1 = *(Process *)this;
  Process p2 = *(Process *)that;
  
  //compare the priorities of this and that
  //If the priority of this is higher, return -1
  if(p1.priority > p2.priority){
    return -1;
  }
  //If the priority of that is higher, return 1
  else if(p1.priority < p2.priority){
    return 1;
  }
   //If the priorities are equal, compare the arrival times
  else {
    //If the arrival time if this is lower, return -1
    if (p1.arrival_time < p2.arrival_time){
      return -1;
    }
    //If the arrival time of that is higher, return 1
    else {
      return 1;
    }
  }
