...
void allocate_memory(list_t * freelist, list_t * alloclist, int pid, int blocksize, int policy) {
   ...
   if(policy == 1){
     if (list_is_in_by_size(freelist, blocksize)){
       temp = list_remove_from_front(freelist);
       temp->pid = pid;
       fragment->pid = 0;
       fragment-> end = temp-> end;
       temp->end = temp->start + blocksize -1;
       fragment->start = temp->end + 1;
       list_add_ascending_by_address(alloclist, temp);
...
void deallocate_memory(list_t * alloclist, list_t * freelist, int pid, int policy) { 
  ...
  if(policy == 1){
    if(list_is_in_by_pid(alloclist, pid)){
      tempIndex = list_get_index_of_by_Pid(alloclist, pid);
      temp = list_remove_at_index(alloclist, tempIndex);
      temp->pid = 0;
      list_add_to_back(freelist, temp);  
    }else{
    ...
    if(list_is_in_by_pid(alloclist, pid)){
      ...
      temp = list_remove_at_index(alloclist, tempIndex);
      temp->pid = 0;
      list_add_ascending_by_blocksize(freelist, temp);  
    }else{
      printf("Error: Can't locate Memory Used by PID: %d \n", pid);
    }
  }
    ...
    if(list_is_in_by_pid(alloclist, pid)){
      ...
      temp = list_remove_at_index(alloclist, tempIndex);
      temp->pid = 0;
      list_add_descending_by_blocksize(freelist, temp);  
    }else{
      printf("Error: Can't locate Memory Used by PID: %d \n", pid);
    }
  }
