...
{ 
  ...
  while(comp != n){
    for(int i = 0; i < n; i++){
      if(plist[i].art <= t && remTime[i] < min && remTime[i] >0){
        min = remTime[i];
        minIndex = i;
        timeCheck = 1;
      }
    ...
    min = remTime[minIndex];
    if(min == 0){
      min = 1000;
    }
    
    if(remTime[minIndex] == 0){
      
      comp += 1;
      timeCheck = 0;
      
      compTime = t + 1;
      plist[minIndex].wt = compTime - plist[minIndex].art - plist[minIndex].bt;
...
{ 
    ...
    ProcessType *temp;
    ProcessType *temp2;
    temp = (ProcessType *)this;
    temp2 = (ProcessType *)that;
    
    if(temp->pri < temp2->pri){
      return 1;
    }
    return 0;
} 
